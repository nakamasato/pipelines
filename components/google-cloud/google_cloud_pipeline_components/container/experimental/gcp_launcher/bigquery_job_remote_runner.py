...
def _poll_query_results(projectId, jobId, creds):
  if not creds.valid:
    creds.refresh(google.auth.transport.requests.Request())
  headers = {
      'Content-type': 'application/json',
...
def bigquery_evaluate_model_job(
  ...
  Args:
      ...
      executor_input:A json serialized pipeline executor input.
  """
  if ((query_statement is not None) and (table_name is not None)):
  ...
  if query_statement is not None:
    query_statement = ', (%s)' % query_statement
  ...
  # Poll bigquery job status until finished.
  job = _poll_job(job_uri, creds)
  query_results = _poll_query_results(project, job['id'], creds)
  artifact_util.update_output_artifact(
      executor_input, 'evaluation_metrics', '',
      {
          _ARTIFACT_PROPERTY_KEY_SCHEMA: query_results['schema'],
          _ARTIFACT_PROPERTY_KEY_ROWS: query_results['rows']
      })
