...
# Tests for evaluate model job.
  ...
  def test_evaluate_model_job_succeeded(self, mock_time_sleep,
                                        ...
                                        mock_get_query_results_requests, _,
                                        mock_auth):
    creds = mock.Mock()
    creds.token = 'fake_token'
    mock_auth.return_value = [creds, 'project']
    ...
    mock_get_requests.return_value = mock_polled_evaluate_model_job
    ...
    mock_get_query_results_requests.return_value = mock_get_query_results
    ...
    mock_get_query_results_requests.assert_called_once_with(
        url=f'https://www.googleapis.com/bigquery/v2/projects/{self._project}/queries/eval_query_id',
        headers={
            'Content-type': 'application/json',
            'Authorization': 'Bearer fake_token',
            'User-Agent': 'google-cloud-pipeline-components'
    ...
    with open(self._output_file_path) as f:
      self.assertEqual(
          f.read(),
          '{"artifacts": {"evaluation_metrics": {"artifacts": [{"metadata": {"schema": "", "rows": ""}, "name": "foobar", "type": {"schemaTitle": "system.Artifact"}, "uri": ""}]}}}'
      )
